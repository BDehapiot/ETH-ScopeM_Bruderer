...
train_type = "rslice" # "rscale" or "rslice"
...
for path in train_path.iterdir():
    if train_type in path.name and 'mask' in path.name:
        ...
        # Open & process data        
        if train_mode == "all":
            msk = get_all(io.imread(path))
            img = io.imread(str(path).replace('_mask', ''))
        if train_mode == "outlines":
            msk = get_outlines(io.imread(path))
            img = io.imread(str(path).replace('_mask', ''))        
...
model_checkpoint_callback = ModelCheckpoint(
    filepath=f"{train_type}_{train_mode}_model_weights_{size:04d}-{overlap:04d}.h5",
